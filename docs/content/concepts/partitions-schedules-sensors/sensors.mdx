...
Once `my_directory_sensor` is added to a <PyObject object="repository"/> with `log_file_job`, it can be started and will start creating runs. You can start or stop sensors in Dagit, or by setting the default status to `SensorStatus.RUNNING` in code:
